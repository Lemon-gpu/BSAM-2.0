...
!
   TYPE, PUBLIC:: node
      ...
      TYPE(node), POINTER:: neighbor ! Nodes on the same level
      INTEGER:: level
      INTEGER:: childno              ! Child's ordinal. Eldest=1, Next=2, ...
      INTEGER:: nrofchildren
      INTEGER:: leafdist             ! Distance from leaf
      INTEGER:: globalnodenum        ! Global node index.
      INTEGER:: levelnodenum         ! Level node index.
   END TYPE node
!
   TYPE:: nodepointer
...
!
   ...
   INTEGER:: currentlevel, globalnodeindex, levelnodeindex
