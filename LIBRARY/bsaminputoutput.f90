...
CHARACTER(LEN=16):: filename, resfilename
INTEGER:: level
!
! Matlab format:
WRITE(filename,'(A7,I5.5,A4)') './OUT/m', nframe, '.dat'
!WRITE(resfilename,'(A7,I5.5,A4)') './OUT/r', nframe, '.dat'
OPEN(UNIT=54,FILE=filename,ACTION='READWRITE',STATUS='REPLACE',FORM='FORMATTED')
WRITE(54,'(F25.12)') time
WRITE(54,'(I3)') finestlevel
!OPEN(UNIT=55,FILE=resfilename,ACTION='READWRITE',STATUS='REPLACE',FORM='FORMATTED')
!WRITE(55,'(F25.12)') time
!WRITE(55,'(I3)') finestlevel
DO level = rootlevel, finestlevel
  CALL ApplyOnLevel(level,OutputQ,dummy)
!  CALL ApplyOnLevel(level,OutputRes,dummy)
END DO
CLOSE(54)
!CLOSE(55)
!
...
!
WRITE(54,*) ' '
WRITE(54,'(I3,3(1x,I3))') info%level, ndims, 2, nccv+ndims*nfcv
!
SELECT CASE(ndims)
  CASE(2)
...
!
! Eventually we may need to separate the outputs of the velocities from the
! output of q:
    CALL WriteQ2D(info%q(  0:mx(1)+1, 0:mx(2)+1,1,1:nccv), &
                  info%v1(-1:mx(1)+1, 0:mx(2)+1,1,1:nfcv), &
                  info%v2( 0:mx(1)+1,-1:mx(2)+1,1,1:nfcv),mx(1:2))
  CASE(3)
    ...
    CALL WriteQ3D(info%q ( 0:mx(1)+1, 0:mx(2)+1, 0:mx(3)+1,1:nccv), &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
SUBROUTINE WriteQ2D(q,v1,v2,mx)
USE NodeInfoDef
IMPLICIT NONE
!
...
!
INTEGER:: i, j, k
REAL(KIND=r8):: rdummy1, rdummy2
!
DO j = 0, mx(2)+1
  DO i = 0, mx(1)+1
!
    rdummy1 = (v1(i,j,1)+v1(i-1,j  ,1))/2.0_r8
    rdummy2 = (v2(i,j,1)+v2(i  ,j-1,1))/2.0_r8
!
    WRITE(54,3001) (q(i,j,k),k=1,nccv), rdummy1, rdummy2
    3001 FORMAT(10(F25.12,2X))
    BACKSPACE(54)
    READ(54,'(F25.12)') rdummy1
!
  END DO
END DO 
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
SUBROUTINE WriteQ3D(q,v1,v2,v3,mx)
USE NodeInfoDef
IMPLICIT NONE
!
...
INTEGER:: i, j, k, l
REAL(KIND=r8):: rdummy
REAL(KIND=r8):: rdummy1, rdummy2, rdummy3
!
DO k = 0, mx(3)+1
  DO j = 0, mx(2)+1
    DO i = 0, mx(1)+1
!
    rdummy1 = (v1(i,j,k,1)+v1(i-1,j  ,k  ,1))/2.0_r8
    rdummy2 = (v2(i,j,k,1)+v2(i  ,j-1,k  ,1))/2.0_r8
    rdummy3 = (v3(i,j,k,1)+v3(i  ,j  ,k-1,1))/2.0_r8
!
      WRITE(54,3001) (q(i,j,k,l),l=1,nccv), rdummy1, rdummy2, rdummy2
      3001 FORMAT(10(F25.12,2X))
      BACKSPACE(54)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
INTEGER FUNCTION OutputRes(info,dummy)
USE NodeinfoDef
USE TreeOps, ONLY: err_ok
IMPLICIT NONE
!
...
!
WRITE(55,*) ' '
WRITE(55,'(I3,3(1x,I3))') info%level, ndims, 2, nccv+ndims*nfcv
!
SELECT CASE(ndims)
  CASE(2)
    WRITE(55,2001) info%dx(1), info%dx(2)
    WRITE(55,2001) info%xlower(1), info%xlower(2)
    WRITE(55,2001) info%xupper(1), info%xupper(2)
    2001 FORMAT(F25.12,1x,F25.12)
    WRITE(55,'(I8,1X,I8)') info%mx(1), info%mx(2)
    WRITE(55,'(I8,3(1X,I8))') info%mglobal(1,1), info%mglobal(1,2), &
                              info%mglobal(2,1), info%mglobal(2,2)
    CALL WriteRes2D(info%rf( 1:mx(1),1:mx(2),1,1:nccv), &
                    info%rf1(0:mx(1),1:mx(2),1,1:nfcv), &
                    info%rf2(1:mx(1),0:mx(2),1,1:nfcv),mx(1:2))
  CASE(3)
    CONTINUE
END SELECT
!
END FUNCTION OutputRes
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
SUBROUTINE WriteRes2D(rf,rf1,rf2,mx)
USE NodeInfoDef
IMPLICIT NONE
!
REAL(KIND=r8), DIMENSION(1:,1:,1:), INTENT(IN):: rf
REAL(KIND=r8), DIMENSION(0:,1:,1:), INTENT(IN):: rf1
REAL(KIND=r8), DIMENSION(1:,0:,1:), INTENT(IN):: rf2
INTEGER, DIMENSION(1:2), INTENT(IN):: mx
!
INTEGER:: i, j, k
REAL(KIND=r8):: rdummy1, rdummy2
...
DO j = 1, mx(2)
  DO i = 1, mx(1)
!
    rdummy1 = (rf1(i,j,1)+rf1(i-1,j,1))/2.0_r8
!
    rdummy2 = (rf2(i,j,1)+rf2(i,j-1,1))/2.0_r8
!
    WRITE(55,3001) (rf(i,j,k),k=1,nccv), rdummy1, rdummy2
    3001 FORMAT(10(F25.12,2X))
    BACKSPACE(55)
    READ(55,'(F25.12)') rdummy1
!
  END DO
END DO 
!
END SUBROUTINE WriteRes2D
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
SUBROUTINE ReadQ
USE NodeInfoDef
...
INTEGER:: ierror, ioerror, level, lvl, restartndims, nnccv, npatch, &
...
!  
    ...
    READ(54,'(I3,3(1x,I3))') level, restartndims, r, nnccv
!
...
!
      IF(nnccv/=nccv) THEN
        PRINT *, 'Readq: Error reading data file ', filename
        PRINT *, 'Number of variables (nccv) inconsistancy on restart.'
        STOP
      END IF
!
...
! Read-in field data:
    SELECT CASE(ndims)
      CASE(2)
        CALL ReadQ2D(info%q(0:mx(1)+1,0:mx(2)+1,        1,1:nccv),mx(1:2))
      CASE(3)
        CALL ReadQ3D(info%q(0:mx(1)+1,0:mx(2)+1,0:mx(3)+1,1:nccv),mx(1:3))
    END SELECT
!
  END DO patch_loop
END DO level_loop
...
DO k = 0, mx(3)+1
  DO j = 0, mx(2)+1
    DO i = 0, mx(1)+1
!
      READ(54,3001) (q(i,j,k,l),l=1,nccv)
      3001 FORMAT(10(F25.12,2X))
!
    END DO
  END DO
...
mx = 1; mx(1:ndims) = rootinfo%mx(1:ndims)
!
CALL AllocUniformGrids(mx)
!
low = 1; low(1:ndims) = 0
high = 1; high(1:ndims) = mx(1:ndims)+1
!
uniformgrid(rootlevel)%q(low(1):high(1),low(2):high(2),low(3):high(3),1:nccv) &
            = rootinfo%q(low(1):high(1),low(2):high(2),low(3):high(3),1:nccv)
!
mxuf = mx
!
...
!
  SELECT CASE(ndims)
    CASE(2)
          uniformgrid(level  )%q(0:mxuf(1)+1,0:mxuf(2)+1,1,1:nccv) &
        = BiLinProlongationP1MC( &
          uniformgrid(level-1)%q(0:mxuc(1)+1,0:mxuc(2)+1,1,1:nccv))
    CASE(3)
          uniformgrid(level  )%q(0:mxuf(1)+1,0:mxuf(2)+1,0:mxuf(3)+1,1:nccv) &
        = TriLinProlongationP1MC( &
          uniformgrid(level-1)%q(0:mxuc(1)+1,0:mxuc(2)+1,0:mxuc(3)+1,1:nccv))
  END SELECT
!
...
WRITE(54,'(I3)') 0
WRITE(54,*) ' '
WRITE(54,'(I3,3(1x,I3))') 0, ndims, 2, nccv
!
SELECT CASE(ndims)
  CASE(2)
...
!
uniformgrid(level)%q(l1(1):h1(1),l1(2):h1(2),l1(3):h1(3),1:nccv) &
  =           info%q(l2(1):h2(1),l2(2):h2(2),l2(3):h2(3),1:nccv)
!
